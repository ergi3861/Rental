...
export function Rental() {
  ...
  const hasSearchParams = pickupDate || dropDate;
  ...
  const handleFilterChange = (key, value) => {
    if (key === "__reset__") { setActiveFilters({}); setCurrentPage(1); return; }
    setCurrentPage(1);
    ...
    <div id="cars">
      ...
      <section className="carsPage">
        <div className="toolbarOuter">
          <h1 className="pageTitle">Makina me qira</h1>

          {hasSearchParams && (
            <div className="searchSummary">
              <span>📍 {pickupLocation || "—"} → {dropLocation || "—"}</span>
              {pickupDate && <span>📅 {pickupDate} {pickupTime}</span>}
              {dropDate   && <span>🔄 {dropDate} {dropTime}</span>}
            </div>
          )}
        ...
        <div className="carsBody">
          <Sidebar filters={filters} activeFilters={activeFilters} onFilterChange={handleFilterChange} />
