...
export default function Sidebar({ filters = {}, onFilterChange = () => {}, activeFilters = {} }) {
    ...
    return(
        <aside id="sidebar">
            ...
            {activeFilters.type && (
                <FilterSection title={`Cmimi ${activeFilters.type === "RENTAL" ? "(Leke/Dite)" : "Euro"}`}>
                    <div className="price">
                        <input type="number" placeholder="Min" min={15}
                            value={activeFilters.minPrice || ""}
                            onChange={(e) => onFilterChange("minPrice", e.target.value)}
                            className="number half" />
                        <input type="number" placeholder="Max" max={500000}
